...
struct MepVal {
    ...
    size_t     str_len = 0;        // Line length
    void*      ptr    = nullptr;   // TYPE == OPAQUE

    MepVal() = default;

    // STRICT COPY (Deep copy of rows, tensors remain references to the pool)
    MepVal(const MepVal& other) {
        copy_from(other);
    }

    MepVal& operator=(const MepVal& other) {
        if (this != &other) {
            _free_str();
            reset_storage();
            copy_from(other);
    ...
    MepVal(MepVal&& other) noexcept {
        move_from(other);
    }

    MepVal& operator=(MepVal&& other) noexcept {
        if (this != &other) {
            _free_str();
            reset_storage();
            move_from(other);
    ...
    // ── setters ──────────────────────────────────────────────────────────────
    void _free_str() {
        if (type == MepValType::STRING && str_p) { free(str_p); str_p = nullptr; str_len = 0; }
    }

    void reset_storage() {
        type = MepValType::NONE;
        tensor = nullptr;
        i64 = 0;
        f64 = 0.0;
        b = false;
    ...
    void copy_from(const MepVal& other) {
        type = other.type;
        tensor = other.tensor; // Tensors are managed by a pool, leaving shallow copying
        i64 = other.i64;
        f64 = other.f64;
        b = other.b;
        ptr = other.ptr;
        ...
        // Strings are allocated via malloc, do a DEEP copy
        if (other.type == MepValType::STRING && other.str_p) {
            str_len = other.str_len;
            str_p = static_cast<char*>(malloc(str_len + 1));
            if (str_p) {
                memcpy(str_p, other.str_p, str_len);
                str_p[str_len] = '\0';
            }
        }
    }

    void move_from(MepVal& other) {
        type = other.type;
        tensor = other.tensor;
        i64 = other.i64;
        f64 = other.f64;
    ...
    std::string to_string() const {
        ...
        switch (type) {
            case MepValType::INT64:   
                snprintf(buf, sizeof(buf), "%lld", (long long)i64); 
            ...
            case MepValType::STRING:  
                return str_p ? std::string(str_p, str_len) : "";
            default:                  
                return "";
        }
    }

    static MepVal from_string(const std::string& s) {
        MepVal v;
        if (s == "True" || s == "true") { v.set_bool(true); return v; }
        if (s == "False" || s == "false") { v.set_bool(false); return v; }
        
        char* end = nullptr;
        long long i_val = std::strtoll(s.c_str(), &end, 10);
        if (end == s.c_str() + s.length()) { v.set_i64(i_val); return v; }
        
        end = nullptr;
        double d_val = std::strtod(s.c_str(), &end);
        if (end == s.c_str() + s.length()) { v.set_f64(d_val); return v; }
        ...
        switch (type) {
            ...
            case MepValType::TENSOR:
                if (tensor && tensor->num_elements >= 1) {
                    switch (tensor->dtype) {
                        case DataType::FLOAT32: return (int64_t)static_cast<float*>(tensor->data)[0];
                        case DataType::INT32:   return (int64_t)static_cast<int32_t*>(tensor->data)[0];
                        case DataType::INT8:    return (int64_t)static_cast<int8_t*>(tensor->data)[0];
                        default: break;
                    }
                }
                return 0;
...
public:
    ...
    /// Sets the execution mode (infer, train, infer_train) requested by SRC_EXEC_MODE
    void set_exec_mode(const std::string& mode) {
        m_exec_mode = mode;
    }
    void set_train_mode(const std::string& mode) { m_train_mode = mode; }

    /// Execute the MEP plan.  Returns the value from EXEC_RETURN, or NONE on HALT.
    MepVal run();

private:
    std::map<uint8_t, NacRuntimeContext*> m_models;
    // ── VM state ──────────────────────────────────────────────────────────────
    ...
    MepVal                     m_return_val;
    std::string                m_exec_mode = "infer_train";

    // Saved copies of small INT32 input tensors (token_ids, etc.) preserved
    // across MODEL_RUN_STATIC for post-run MEP ops (e.g. RoBERTa LOGIC_COMPARE).
    // Stored in plain std::vector — NOT in the tensor pool — to avoid any
    // interference with PRELOAD or pool allocations during the model run.
    struct SavedInput {
        uint8_t              mep_key;
        DataType             dtype;
        std::vector<int>     shape;
        std::vector<uint8_t> raw_data; // byte-for-byte copy of tensor->data
    };
    std::vector<SavedInput> m_saved_inputs;
    ...
    void h_src_exec_mode();      // 0x05
    void h_res_load_array();     // 0x14

    // Training stubs (0x82, 0x83, 0x85)
    void h_model_train_step();
    void h_model_zero_grad();
    void h_model_save_weights();
     ...
     */
    bool run_model_sync(NacRuntimeContext* ctx, std::vector<Tensor*>& out_tensors);

    /// Inline memory-task tick: processes mmap_schedule commands for `tick`.
    void mmap_tick_sync(NacRuntimeContext* ctx, uint32_t tick); 

    /// Load a weight tensor on-demand (shared with mmap preload path).
    Tensor* load_param_tensor(NacRuntimeContext* ctx, uint16_t param_id);

    std::string m_train_mode = "head_only";
    void execute_trng_block(NacRuntimeContext* ctx, Tensor* grad_logits);
};
